...
func TestEncodeTakerTraits(t *testing.T) {
	...
	takerTraits := limitorder.NewDefaultTakerTraits()
	takerTraits.SetExtension(extension).SetAmountMode(limitorder.AmountModeMaker).SetAmountThreshold(big.NewInt(1))
...
func TestDecodeArgs(t *testing.T) {
	...
	t.Run("full decode", func(t *testing.T) {
		...
		takerTraits := limitorder.NewTakerTraits(big.NewInt(0), &receiver, &extension, &interaction)
		flags, args := takerTraits.Encode()

		// Decode back
		decodedReceiver, decodedExtension, decodedInteraction, err := limitorder.DecodeArgs(flags, args)
	...
	t.Run("receiver only", func(t *testing.T) {
		...
		takerTraits := limitorder.NewTakerTraits(big.NewInt(0), &receiver, nil, nil)
		flags, args := takerTraits.Encode()
	...
	t.Run("extension only", func(t *testing.T) {
		...
		takerTraits := limitorder.NewTakerTraits(big.NewInt(0), nil, &extension, nil)
		flags, args := takerTraits.Encode()
	...
	t.Run("interaction only", func(t *testing.T) {
		...
		takerTraits := limitorder.NewTakerTraits(big.NewInt(0), nil, nil, &interaction)
		flags, args := takerTraits.Encode()
